-- Lead Intelligence Network: new tables + permits alterations
...
create table if not exists gc_profiles (
  ...
  first_seen date,
  last_seen date,
  top_addresses jsonb default '[]'::jsonb,
...
-- RLS policies (match existing permits pattern: service role has full access)
...
-- Service role can do everything (handled by default with service_role key)
